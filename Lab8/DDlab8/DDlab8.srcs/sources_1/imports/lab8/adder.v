...
always@(posedge clk) begin
  ...
  state <= (counter==100000) ? (state + 1) : state;
   case(state)
	0:begin
	  seg_number <= first_number/100;//顯示first_number百位數
	  scan <= 8'b0111_1111;//0 代表顯示該7段顯示器
	end
	1:begin
	  seg_number <= (first_number%100)/10;//顯示first_number十位數
	  scan <= 8'b1011_1111;
	end
	2:begin
	  seg_number <= first_number%10;//顯示first_number個位數
	  scan <= 8'b1101_1111;
	end
	3:begin
	  seg_number <= second_number/10;//顯示second_number十位數
	  scan <= 8'b1110_1111;
	end
	4:begin
	  seg_number <= second_number%10;//顯示second_number個位數
	  scan <= 8'b1111_0111;
	end
	5:begin
	  seg_number <= answer_number/100;//顯示answer_number百位數
	  scan <= 8'b1111_1011;
	end
	6:begin
