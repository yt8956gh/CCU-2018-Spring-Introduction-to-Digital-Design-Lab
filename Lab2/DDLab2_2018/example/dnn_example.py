...
def inference(images, hidden1_units):
    with tf.name_scope('nodes_hidden1'):
        weights1 = tf.Variable(tf.truncated_normal(
            [IMAGE_PIXELS, hidden1_units], stddev=1.0 / math.sqrt(float(IMAGE_PIXELS)), dtype=tf.float32), name='weights1')
        biases1 = tf.Variable(
            tf.zeros([hidden1_units], dtype=tf.float32), name='biases1')
        hidden1 = tf.nn.relu(tf.matmul(images, weights1) + biases1)

    with tf.name_scope('nodes_output_layer'):
        weightso = tf.Variable(tf.truncated_normal(
            [hidden1_units, NUM_CLASSES], stddev=1.0 / math.sqrt(float(hidden1_units)), dtype=tf.float32), name='weightso')
        biaseso = tf.Variable(
            tf.zeros([NUM_CLASSES], dtype=tf.float32), name='biaseso')
            ...
            if len(shape) > 1:
                for v_ in value:
                    for w in v_.tolist():
                        ...
                        binary_str = ''.join('{:02x}'.format(x)
                                             for x in reversed(binary_value))
